...
##'
##' To save time and typing, this function will load RData files from /cache
##' directory of project tree by name.
##' @title Load cached objects from /cache directory of project tree
##' @param str Name of object saved RData file
...
##' @export
thaw <- function(str) {
  p <- paste(file.path(getwd(), "cache", str), ".RData", sep = "")
  if (file.exists(p)) {
